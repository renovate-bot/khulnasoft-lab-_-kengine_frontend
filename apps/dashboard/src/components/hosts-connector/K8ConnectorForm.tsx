...
const defaultNamespace = 'kengine';
...
const InformationForm = ({
  ...
  useMemo(() => {
    ...
    const installCommand = `helm install kengine-agent kengine/kengine-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set kengineKey=${dfApiKey} \\
  ...
  return (
    <div className="relative flex items-center">
      <pre className="h-fit text-p7a text-text-text-and-icon">
        helm repo add kengine
        https://kengine-helm-charts.s3.amazonaws.com/threatmapper
      </pre>
      <div className="flex items-center ml-auto self-start">
        ...
        <IconButton
          ...
          onClick={() => {
            copy(
              'helm repo add kengine https://kengine-helm-charts.s3.amazonaws.com/threatmapper',
            );
          }}
        />
      </div>
...
export const K8ConnectorForm = () => {
  const [instruction, setInstruction] =
    useState(`helm repo add kengine https://kengine-helm-charts.s3.amazonaws.com/threatmapper
helm repo update

helm install kengine-agent kengine/kengine-agent \\
  --set managementConsoleUrl=${window.location.host ?? '---CONSOLE-IP---'} \\
  --set kengineKey=${PLACEHOLDER_API_KEY} \\
  --set global.imageTag=${''} \\
      ...
      <Stepper>
        ...
        >
          <div className="text-p7a text-text-text-and-icon">
            ...
            <DFLink
              href={`https://community.khulnasoft.com/threatmapper/docs/v2.2/sensors/kubernetes`}
              target="_blank"
              rel="noreferrer"
              className="mt-2"
            >
